...
export function TeamsContent() {
  ...
  const generateInvite = async () => {
    ...
    const { data, error } = await supabase
      .from("invitations")
      .insert({ org_id: org.id, role: inviteRole, invited_by: user.id })
      .select()
      .single();
  ...
  useEffect(() => {
    const handle = requestAnimationFrame(() => loadData());
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <header className="flex items-end justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-zinc-500 font-bold uppercase tracking-widest">
          <span className="h-2 w-2 rounded-full bg-[#51DD7D] animate-pulse" />
          ...
          <CardContent>
            <div className="space-y-3">
              {members.map((m) => (
                <div
                  key={m.id}
                  className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl group hover:bg-white/[0.04] transition-colors"
                >
